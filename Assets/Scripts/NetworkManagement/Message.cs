...
namespace GameServer {
    public class Message {
        ...
        public static byte[] BuildDataPackage(int Crccode, long sessionid, int command, int subcommand, int encrypt,
            string[] MessageBody) {
            //消息校验码默认值为0x99FF
            Crccode = 65433;
        ...
        ///接收消息
        ///</summary>
        public static string ReceiveMessage(Socket socketClient) {
            while (true) {
                ...
                //一个消息包接收完毕，解析消息包
                return UnpackData(recvBytesHead, recvBytesBody);
            }
        }
...
//                Debug.Log("接收到数据包中的数据有：" + Encoding.UTF8.GetString(_byte, 0, _byte.Length));
                return Encoding.UTF8.GetString(_byte, 0, _byte.Length);
            }
            Debug.Log("不对");
            return "";
        }
